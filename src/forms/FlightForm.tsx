...
export default function FlightForm({ onFormSubmit }: { onFormSubmit?: (arg: any) => void }) {
  const navigate = useNavigate();
  const smallScreen = useMediaQuery('(min-width: 640px)');
  const [passengers, setPassengers] = useState({totalCount: 1});
  ...
  const handleSubmit = (values: any) => {
    const formData = { ...values, passengers: passengers}
          ...
          <PassengerDropdown
            buttonComponent={
              <span className='cursor-pointer flex items-center  gap-x-3'>
                <p className='text-gray-600 !font-medium'>{passengers.totalCount} passenger(s)</p>
            ...
            onStateChange={(arg: any) => setPassengers(arg)}
          />
        </div>
